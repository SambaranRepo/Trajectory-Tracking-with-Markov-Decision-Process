...
def casadi_controller(cur_state, ref):
     ...
     for i in range(1,N+1):
          ...
          hcat([0, 0.5])) @ U[:,i-1] \
               - vcat([ref[i][0] - ref[i-1][0], ref[i][1] - ref[i-1][1], (ref[i][2] - ref[i-1][2] + np.pi) % (2 * np.pi) - np.pi]))
          opti.subject_to(opti.bounded(vcat([-2.5,-2.5]), E[:2, i] + vcat([ref[i][0], ref[i][1]]), vcat([2.5,2.5])))
          opti.subject_to((E[0,i] + + ref[i][0] +  2)**2 + (E[1,i]+ ref[i][1] + 2)**2 > 0.5**2)
          opti.subject_to((E[0,i] + ref[i][0]- 1)**2 + (E[1,i] + ref[i][1] - 2)**2 > 0.5**2)
